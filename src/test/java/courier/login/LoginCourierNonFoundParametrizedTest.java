...
public class LoginCourierNonFoundParametrizedTest {
    private final static String MESSAGE_NOT_FOUND = "Учетная запись не найдена";
    ...
    public LoginCourierNonFoundParametrizedTest(final String login, final String password) {
        this.login = login;
    ...
    public static Object[][] textData() {
        return new Object[][]{
                {"6666", RANDOM_STRING_PASSWORD},
                {RANDOM_STRING_LOGIN, "00001"}
        };
    }
    ...
    @Test
    @DisplayName("Логин курьера в системе - авторизация с некорректными данными")
    @Description("Проверка авторизации c некорректными логином или паролем")
    public void checkLoginNonExisting() {
        specification.loginCourier(new CourierLoginDto(login, password))
                .then().assertThat().body("message", equalTo(MESSAGE_NOT_FOUND))
                .and()
